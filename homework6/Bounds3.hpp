                                ...
                                const std::array<int, 3>& dirIsNeg) const
{
    ...
    if (dir.x < 0) std::swap(tx_min, tx_max);
    if (dir.y < 0) std::swap(ty_min, ty_max);
    if (dir.z < 0) std::swap(tz_min, tz_max);
    
    float tenter = fmax(tx_min, fmax(ty_min, tz_min));
    float texit = fmin(tx_max, fmin(ty_max, tz_max));
